...
// Quest tiles → open preview or go to quests page
document.addEventListener('click', (e) => {
    const tile = e.target.closest('[data-quest-link]');
    if (!tile) return;
    e.preventDefault();
    if (!window.currentUser) {
        if (typeof window.openAuthModal === 'function') window.openAuthModal('register');
    ...
    const questId = tile.dataset.questLink;
    if (questId && typeof window.openQuestPreview === 'function') {
        if (typeof window.showPage === 'function') window.showPage('quests');
        window.openQuestPreview(questId).catch(() => {});
        return;
    }
    if (typeof window.showPage === 'function') window.showPage('quests');
    window.scrollTo({ top: 0, behavior: 'smooth' });
