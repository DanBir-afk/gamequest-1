...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getEarlyLocalXp(user) {
        if (!user?.id) return parseInt(localStorage.getItem('gamequest_xp') || '0', 10) || 0;
        const perUser = parseInt(localStorage.getItem(`gamequest_${user.id}_xp`) || '0', 10) || 0;
        const legacy = parseInt(localStorage.getItem('gamequest_xp') || '0', 10) || 0;
        return Math.max(perUser, legacy);
    }

    function setEarlyProgress(totalXp, isLoggedIn) {
        ...
        if (xpEl) xpEl.textContent = `XP: ${xpIntoLevel.toLocaleString('en-US')} XP / 1,000 XP`;
        if (progressFill) progressFill.style.width = `${Math.min(100, (xpIntoLevel / 1000) * 100)}%`;
    ...
    async function saveEarlyProgressToSupabase(client, user, profile, totalXp) {
        if (!client || !user) return false;
        const xp = Math.max(0, parseInt(totalXp, 10) || 0);
        const level = Math.floor(xp / 1000);
        const payload = { xp, level };
        ...
        try {
            ...
            console.warn('Supabase XP update by user_id failed:', err);
        }
        ...
        try {
            ...
            console.error('Supabase XP upsert failed:', err);
            showAuthMessage('error', 'Supabase blocked XP saving. Check profiles RLS policies.');
        }
    ...
    function applyEarlyAuthState(user, profile) {
        ...
        if (profileName && isLoggedIn) profileName.textContent = name;

        const usernameInput = document.getElementById('username');
        if (usernameInput && isLoggedIn) usernameInput.value = name;

        if (isLoggedIn) {
            const localXp = getEarlyLocalXp(user);
            const profileXp = Number(profile?.xp ?? profile?.experience ?? profile?.points ?? 0) || 0;
            const xp = Math.max(localXp, profileXp);
            setEarlyProgress(xp, true);
            if (user?.id) {
                localStorage.setItem(`gamequest_${user.id}_xp`, String(xp));
            }
            if (xp > profileXp) {
                const client = typeof initSupabaseClient === 'function' ? initSupabaseClient() : null;
                saveEarlyProgressToSupabase(client, user, profile, xp).then(() => setEarlyProgress(xp, true));
            }
        } else {
            setEarlyProgress(0, false);
        }
    }
