...
function updateHeaderPlayerStats(profile, isLoggedIn) {
    ...
    const levelFromPage = parseStatNumber(document.querySelector('.profile-level')?.textContent, 0);
    const xpFromPage = parseStatNumber(document.querySelector('.profile-xp')?.textContent, 0);
    const profileXp = parseStatNumber(profileStat(profile, ['xp', 'experience', 'points'], null), null);
    const xp = xpFromPage || profileXp || 0;
    const level = levelFromPage || Math.floor((profileXp || xp) / 1000);
...
function renderUserWidget(user, profile) {
    ...
    const localXp = parseInt(localStorage.getItem(window.playerStorageKey('xp')) || '0', 10);
    const profileXp = Number(profile?.xp ?? profile?.experience ?? profile?.points ?? 0) || 0;
    if (localXp > profileXp) {
        window.savePlayerProgressToSupabase(localXp).then(() => window.updateProfileXP());
    }
    ...
    // Sync profile page with the user's saved account settings (name, bio, avatar, favorite games)
...
function renderGuestWidget() {
    ...
    // Hide logout button (just in case profile is visible somehow)
    const logoutBtn = document.getElementById('profile-logout-btn');
